...
// Scanner represents a lexical scanner for samql.
type Scanner struct {
	r *reader
}

// NewScanner returns a new instance of Scanner that reads from r.
func NewScanner(r io.Reader) *Scanner {
	return &Scanner{r: &reader{r: bufio.NewReader(r)}}
}

// Scan returns the next token and position from the underlying reader. Also
// returns the literal text read for string and number tokens since these
// token types can have different literal representations.
func (s *Scanner) Scan() (tok Token, pos Pos, lit string) {
	// Read next code point.
	ch0, pos := s.r.read()

	// If we see whitespace then consume all contiguous whitespace. If we see
	...
	// Otherwise parse based on individual characters.
	switch ch0 {
	case eof:
		return EOF, pos, ""
	case '"':
...
// scanIdent consumes an identifier. If lookup is true it checks if the
// literal matches a keyword and returns it.
...
func (s *Scanner) scanRegex() (scanRegex Token, pos Pos, lit string) {
	_, pos = s.r.curr()
...
func scanDelimited(r io.RuneScanner, start, end rune, escapes map[rune]rune,
	escapesPassThru bool) ([]byte, error) {
